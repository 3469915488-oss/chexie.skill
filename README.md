...
```
## Chexie Knowledge
车协相关问题使用 `python search_chexie.py "问题" --top-k 8 --json` 检索后回答。
必须附来源：版面、标题、楼层、作者、时间、链接。
```

**OpenAI Codex CLI**

```bash
codex exec "用 chexie 知识库查一下往年远征路线选择"
```

**OpenAI GPTs / 兼容平台**

`agents/openai.yaml` 提供 GPT Action 配置模板。

更多集成细节见各平台的 AGENTS.md / CONFIGS 文件。
